...
# ─────────────────────────────────────────────────────────────
# Create backup (git branch + filesystem snapshot of framework files)
# ─────────────────────────────────────────────────────────────
header "Creating Backup"

# 1. Filesystem snapshot — always taken, even for non-git projects, so every
#    file is backed up before any migration modifies or deletes it.
BACKUP_DIR="$PROJECT_DIR/.framework-backups/pre-upgrade-v${CURRENT_VERSION}-$(date +%Y%m%d%H%M%S)"
mkdir -p "$BACKUP_DIR"
BACKED_UP=0
if [[ -f "$PROJECT_DIR/.framework-manifest" ]]; then
  # Back up exactly the framework-installed files.
  while IFS= read -r rel; do
    [[ -n "$rel" ]] || continue
    [[ -e "$PROJECT_DIR/$rel" ]] || continue
    mkdir -p "$BACKUP_DIR/$(dirname "$rel")"
    cp -a "$PROJECT_DIR/$rel" "$BACKUP_DIR/$rel" 2>/dev/null && BACKED_UP=$((BACKED_UP + 1))
  done < "$PROJECT_DIR/.framework-manifest"
else
  # No manifest — snapshot the known framework path set.
  for rel in .agents .copilot .contracts .requirements .decisions .repo-index.yml \
             .framework-version .gitmodules .github/agents .github/skills \
             .github/copilot-instructions.md .github/mcp.json; do
    [[ -e "$PROJECT_DIR/$rel" ]] || continue
    mkdir -p "$BACKUP_DIR/$(dirname "$rel")"
    cp -a "$PROJECT_DIR/$rel" "$BACKUP_DIR/$rel" 2>/dev/null && BACKED_UP=$((BACKED_UP + 1))
  done
fi
log "Filesystem backup: $BACKED_UP framework path(s) → .framework-backups/$(basename "$BACKUP_DIR")/"

# 2. Git backup branch (project repo) for easy rollback.
BACKUP_BRANCH="pre-upgrade-v${CURRENT_VERSION}-$(date +%Y%m%d%H%M%S)"
if git -C "$PROJECT_DIR" rev-parse --git-dir &>/dev/null; then
  # Ensure clean working tree (in the PROJECT repo, not the framework repo).
  ...
  git -C "$PROJECT_DIR" branch "$BACKUP_BRANCH" 2>/dev/null || true
  log "Backup branch created: $BACKUP_BRANCH"
else
  warn "Not a git repo — relying on filesystem backup ($BACKUP_DIR)"
fi
...
for migration_file in "${MIGRATION_CHAIN[@]}"; do
  ...
  if ! bash "$migration_file"; then
    ...
    Your filesystem backup is: $BACKUP_DIR
    Your backup branch is: $BACKUP_BRANCH
    To rollback: git -C \"$PROJECT_DIR\" checkout $BACKUP_BRANCH && git -C \"$PROJECT_DIR\" checkout -B $(git -C "$PROJECT_DIR" branch --show-current 2>/dev/null || echo main)"
  fi
...
if git -C "$PROJECT_DIR" rev-parse --git-dir &>/dev/null; then
  ...
  git -C "$PROJECT_DIR" commit -m "chore: upgrade enterprise-copilot-fleet-controller v${CURRENT_VERSION} → v${LATEST_VERSION}
...
echo "  Backup files:  .framework-backups/$(basename "$BACKUP_DIR")/"
echo ""
...
echo "    git -C \"$PROJECT_DIR\" checkout $BACKUP_BRANCH"
echo "    git -C \"$PROJECT_DIR\" checkout -B $(git -C "$PROJECT_DIR" branch --show-current 2>/dev/null || echo main)"
echo ""
