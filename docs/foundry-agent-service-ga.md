<!--
Pinned snapshot for pattern: azure-fabric-fullstack (docs.local_path)
Source: https://devblogs.microsoft.com/foundry/foundry-agent-service-ga/
Fetched: 2026-07-10
Update by re-fetching the source URL. Referenced from patterns/azure-fabric-fullstack/pattern.yml.
-->
...
For key-based auth, add a Custom Keys connection in your Foundry project with an `Authorization` header, then reference it via `project_connection_id`:

```python
from azure.ai.projects.models import MCPTool, PromptAgentDefinition

# project_connection_id: resource ID of a Custom Keys connection
# storing an Authorization header
tool = MCPTool(
...
The Foundry portal (https://ai.azure.com) has an updated agents experience with visual workflow building, a unified Tools tab for MCP, A2A, and Azure AI Search connections, and the separated v1/v2 resource view. If you're coming from Foundry Classic, the new experience is the default.

For a hands-on introduction, the agents quickstart
(https://learn.microsoft.com/azure/foundry/quickstarts/get-started-code) takes you from
zero to a running, tool-using agent in a few minutes.
